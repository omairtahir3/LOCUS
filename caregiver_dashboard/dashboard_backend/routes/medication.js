...
router.get('/schedule/today', caregiverAccessCheck, async (req, res) => {
  try {
    ...
    const meds = await Medication.find({ user_id: userId, is_active: true });
    ...
    const logMap = {};
    logs.forEach(l => { logMap[`${l.medication_id}_${new Date(l.scheduled_time).toTimeString().slice(0,5)}`] = l; });
